...
namespace
{
    ...
    void touch_polling_task(void *arg)
    {
        ...
        while (true)
        {
            // 轮询触摸状态
            bool touched = false;
            esp_err_t ret = esp_lcd_touch_get_state(s_touch_handle, &touched, nullptr, nullptr);
            if (ret != ESP_OK)
            {
                ESP_LOGE(TAG, "Failed to get touch state: %s", esp_err_to_name(ret));
            ...
            if (touched)
            {
                ...
                uint16_t x = 0, y = 0;
                uint8_t touch_cnt = 0;
                ret = esp_lcd_touch_get_coordinates(s_touch_handle, &x, &y, &touch_cnt, 1);
